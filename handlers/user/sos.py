...
async def cmd_sos(message: Message):
    ...
    message.bot.logger.log_info(
        content=f'Запросил помощи')
...
async def process_submit(message: Message, state: FSMContext):
    ...
    if db.fetchone('SELECT * FROM questions WHERE cid=?', (cid,)) == None:
        ...
        message.bot.logger.log_success(
            content=f'Успешно отправлен вопрос в бд')
        async with state.proxy() as data:
        ...
        message.bot.logger.log_error(
            content=f'Превышен лимит на количество задаваемых вопросов')
